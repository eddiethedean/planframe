...
- **CompileExprContext**: Holds the active `Schema` for an expression compile step, plus an optional `resolve_backend_dtype` callback populated by `execute_plan` when native column metadata should augment a partial step schema; passed to `BaseAdapter.compile_expr` and `BaseAdapter.resolve_dtype` (re-exported from `planframe`). Adapters may implement `resolve_backend_dtype_from_frame` to supply dtypes from the live backend frame. Different from **PlanCompileContext** (`planframe.compile_context`), which pairs an adapter with a schema for plan-level compilation helpers used by `Frame` and `execute_plan`.
