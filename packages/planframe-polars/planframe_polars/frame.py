...
PolarsData = dict[str, list[object]] | list[dict[str, object]]
...
def _fill_missing_from_defaults(data: PolarsData, *, defaults: dict[str, object]) -> PolarsData:
    ...
    if isinstance(data, dict):
        ...
        n = len(first)
        out: dict[str, list[object]] = {k: list(v) for k, v in data.items()}
...
def _to_polars_backend_frame(data: PolarsData, *, schema: type[Any], lazy: bool) -> PolarsBackendFrame:
...
class _PolarsFrameMeta(type):
    def __call__(cls, *args: Any, **kwargs: Any) -> Any:
        ...
        return PolarsFrame.source(df, adapter=PolarsFrame._adapter_singleton, schema=cls)


class PolarsFrame(Frame[SchemaT, PolarsBackendFrame, pl.Expr], Generic[SchemaT], metaclass=_PolarsFrameMeta):
    ...
    @classmethod
    def read_database(cls, query: str, *, connection: Any, schema: type[SchemaT]) -> PolarsFrame[SchemaT]:
