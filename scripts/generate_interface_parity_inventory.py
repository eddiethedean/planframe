...
def _build_spark_parent() -> tuple[str, Any]:
    try:
        mod = importlib.import_module("pyspark.sql")
        SparkDataFrame = getattr(mod, "DataFrame", None)
        if SparkDataFrame is None:
            return "pyspark.sql.DataFrame", None
        return "pyspark.sql.DataFrame", SparkDataFrame
    except Exception:
        return "pyspark.sql.DataFrame", None
...
def _render_missing(title: str, rows: list[MissingRow]) -> str:
    ...
    lines.append(f"## {title}")
    lines.append("")
    lines.append("| Parent method | Parent signature | Notes |")
    lines.append("| --- | --- | --- |")
    for r in sorted(rows, key=lambda x: x.name):
        lines.append(
            "| "
...
def main() -> int:
    ...
    spark_parent_name, spark_parent = _build_spark_parent()
    ...
    pandas_missing: list[MissingRow] = []
    if pandas_parent is not None:
        pandas_names = _public_callable_names(pandas_parent)
        our_names_pandas = set(pandas_methods)
        for name in sorted(pandas_names - our_names_pandas):
            pandas_missing.append(
                MissingRow(
                    name=name,
                    parent=pandas_parent_name,
                    parent_sig=_parent_sig(pandas_parent, name),
                    notes="Not implemented in PlanFrame pandas skin `PandasLikeFrame` (yet).",
                )
            )
    ...
    for name, sig in spark_methods.items():
        ...
        spark_rows.append(
            MethodRow(
                name=name,
                our_sig=sig,
                parent=spark_parent_name,
                parent_sig=_parent_sig(spark_parent, name) if spark_parent is not None else "",
                status=status,
                notes=notes,
            )
        )

    spark_missing: list[MissingRow] = []
    if spark_parent is not None:
        spark_names = _public_callable_names(spark_parent)
        our_names2 = set(spark_methods)
        for name in sorted(spark_names - our_names2):
            spark_missing.append(
                MissingRow(
                    name=name,
                    parent=spark_parent_name,
                    parent_sig=_parent_sig(spark_parent, name),
                    notes="Not implemented in PlanFrame spark skin `SparkFrame` (yet).",
                )
            )
    ...
    (out_dir / "pandas-missing.md").write_text(
        ...
        + (
            ...
            if pandas_missing
            else "pandas is not installed (or could not be imported), so this report is empty.\n"
        ),
        encoding="utf-8",
    )
    ...
    (out_dir / "spark-missing.md").write_text(
        ...
        + (
            _render_missing(
                "`pyspark.sql.DataFrame` methods not present on PlanFrame `SparkFrame`",
                spark_missing,
            )
            if spark_missing
            else "PySpark is not installed (or could not be imported), so this report is empty.\n"
        ),
        encoding="utf-8",
    )
